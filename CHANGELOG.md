...
- `apollo-datasource-rest`: The `DataSource` initializer (which is invoked automatically by Apollo Server's request pipeline on provided `dataSources`, will now accept (and receive, by default) a `logger` property which will be used for the output from the `trace` method when running in environments where `NODE_ENV` is set to `development`.
